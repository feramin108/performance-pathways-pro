...
export function SecondaryStep({ entries, setEntries }: Props) {
  const addEntry = () => {
  ...
  return (
    <div>
      <div className="mb-1 flex items-center justify-between">
        <h2 className="text-lg font-semibold tracking-tight">Secondary KPIs</h2>
        <span className="text-data rounded-sm bg-primary/10 px-2 py-0.5 text-xs font-semibold text-primary">
          Weight: 10%
        </span>
      </div>
      <p className="mb-4 text-xs text-muted-foreground">
        Minimum 2, maximum 3 KPIs.
      </p>

      <div className="space-y-3">
        {entries.map((entry, index) => (
          <div key={entry.id} className="relative">
            <KPICard entry={entry} index={index} onChange={(u) => updateEntry(entry.id, u)} />
            {entries.length > 2 && (
